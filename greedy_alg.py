...
def node_process_greedy_collaborative(node, shared_state, lock):
    # Node processes using the shared state
    Co, Es_Con, Es_Dis = node.node_process_greedy(shared_state['APs'], shared_state['parameters_values'])
    
    with lock:
        # Update shared state with the results
        shared_state['results'][node.node_id] = (Co, Es_Con, Es_Dis)

def greedy_alg(sets_count, parameters_values, forecast_horizon):
    """
    Perform decentralized processing using a greedy algorithm with parallel processing.
    """
    N_I = sets_count['N_I']
    N_T = sets_count['N_T']
    N_O = sets_count['N_O']
    ...
    threads = []
    for node in Nodes:
        t = Thread(target=node_process_greedy_collaborative, args=(node, shared_state, lock))
        t.start()
        threads.append(t)
    ...
    for node_id, (Co, Es_Con, Es_Dis) in shared_state['results'].items():
